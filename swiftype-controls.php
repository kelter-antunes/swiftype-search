<?php
	...
	//get realtyme number of documents in index
	$st = new SwiftypePlugin;
	$st->initialize_api_client();

	$args = array(
		'posts_per_page' => -1,
		'meta_key' => 'swiftype_imported',
		'post_type' => $this->allowed_post_types(),
		'meta_value' => 1
	);
	$posts = get_posts($args);
	$num_send_for_crawling = count($posts);
	...
	$blog_posts = get_posts( $posts_query );
	$num_blog_posts = count($blog_posts);
...
try {
	...
	//var_dump( $st->num_indexed_documents);
	...
	<table class="widefat" style="width: 650px;">
		...
		<tbody>
			...
			<tr>
				...
				<td><span id="num_blog_documents"><?php print( $num_blog_posts ); ?></span></td>
			</tr>
			<tr>
				<td>Number of Blog Posts sent to index:</td>
				<td><span id="num_send_to_crawl_documents"><?php print( $num_send_for_crawling ); ?></span></td>
			</tr>
...
//		var total_ops = total_posts + total_posts_in_trash;
		...
		jQuery('#num_send_to_crawl_documents').html(total_posts_written);
