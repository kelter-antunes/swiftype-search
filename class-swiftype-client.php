...
/**
	...
	*/
	...
	/*
	 * Get list of url if engine is crawl type
	 */
	public function get_urls( $engine_id ) {
		$url = $this->endpoint .'engines/'.$engine_id.'/domains.json';
		$response = $this->call_api( 'GET', $url );
		return json_decode( $response['body'], true );
	}
	...
	/*
	 * Crawl a single URL
	 * You may trigger the crawl of a single URL by using the domain's crawl_url endpoint with the URL as a parameter.
	 * If the URL exists in the domain, it will be updated.
	 * If the URL does not exist in the domain, it will be added.
	 */
	public function crawl_url( $engine_id, $domain_id, $new_url ) {
		$url = $this->endpoint .'engines/'.$engine_id.'/domains/'.$domain_id.'/crawl_url.json';
		$params = array( 'url' => $new_url );
		//var_dump($url,$params);
		$response = $this->call_api( 'PUT', $url, $params );
