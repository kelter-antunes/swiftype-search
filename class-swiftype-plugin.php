...
/**
	...
	*/
	...
	class SwiftypePlugin {
		...
		private $document_type_slug = 'page';
	 ...
	 */
		...
		public function can_we_search( $wp_query ) {
			if ( function_exists( 'is_main_query' ) && ! $wp_query->is_main_query() ) {
				return false;
			} elseif ( is_search() && ! is_admin() && $this->engine_slug && strlen( $this->engine_slug ) > 0 ) {
				return true;
			} else {
				return false;
			}
	...
	/**
		...
		*/
		public function get_posts_from_swiftype( $wp_query ) {
			...
			if( $this->can_we_search( $wp_query ) ) {
				...
				$this->search_swiftype( $query_string, $params );
		...
		**/ 
		public function extract_and_save_ids() {
			$this->post_ids = array();
			$records = $this->results['records']['page'];
			
			foreach( $records as $record ) {
				$this->post_ids[] = $record['external_id'];
			}

			$result_info = $this->results['info']['page'];
		...
		**/
		...
		/**
		* Actually do the search using the Swiftype client.
		**/
	...
	/**
		...
		*/
		public function initialize_engine( $engine_name ) {
			//no need to create new engine
//			$engine = $this->client->create_engine( array( 'name' => $engine_name ) );

			$st = new SwiftypePlugin;
			$st->initialize_api_client();
			if ($engine_name!='') {
				$engine = $st->client()->find_engine($engine_name);
				...
				try {
					$document_type = $st->client->find_document_type( $this->engine_slug, $this->document_type_slug );
					$st->num_indexed_documents = $document_type['document_count'];
					$num_indexed_documents = $st->num_indexed_documents;
				} catch( SwiftypeError $e ) {
				}
...
//			update_option( 'swiftype_engine_initialized', $this->engine_initialized );
				update_option('swiftype_engine_initialized', 1);
			}
		}
	...
	/**
		...
		*/
		...
		public function index_batch_of_posts( $offset = 0, $batch_size = 10 ) {
			$posts_query = array(
			...
			$domains = $this->client->get_urls($st->engine_slug);
			if (sizeof($domains) > 0){
				$total = 0;
				//var_dump($domains[0]["id"]);
				foreach( $posts as $post ) {
					if (get_post_meta($post->ID,'swiftype_imported',true)!=1) {
//						var_dump($post->ID);
						$domain_id = $domains[0]["id"];
						$url = $post->guid;
//					$url = get_permalink($post->ID);
						$responce = array();
						while( is_null($responce['external_id'])) {
							try {
								$responce = $this->client->crawl_url($st->engine_slug, $domain_id, $url);
							} catch (SwiftypeError $e) {
								$responce = array();
						...
						if (sizeof($responce) > 0 && $responce['external_id'] != '') {
							$total++;
							update_post_meta($post->ID, 'swiftype_imported', '1');
						}
					} else {$total++;}
				}
				if (sizeof($responce) > 0){
					$document_type = $st->client->find_document_type( $st->engine_slug, $st->document_type_slug );
					$st->num_indexed_documents = $document_type['document_count'];
					$num_indexed_documents = $st->num_indexed_documents;
				}
				return array($total , $num_indexed_documents + $total);
			}



//			var_dump($this->client->get_urls($this->engine_slug));
			//die;
		...
		*/
		public function index_post( $post_id ) {
			...
			try {
				...
				$domains = $this->client->get_urls($this->engine_slug);
				if (sizeof($domains) > 0){
					$url = get_permalink($post->ID);
					$domain_id = $domains[0]["id"];
					$responce = $this->client->crawl_url( $this->engine_slug, $domain_id, $url );
					if (sizeof($responce) > 0 && $responce['external_id'] != '') {
						update_post_meta($post->ID, 'swiftype_imported', '1');
					}
					//var_dump($this->engine_slug, $domain_id, $url,$responce);
	...
	/**
		...
		*/
		public function enqueue_swiftype_assets() {
			...
			wp_register_script( 'swiftype', plugins_url( 'assets/install_swiftype.js', __FILE__ )  );

			$l10n_data = array(
				'all_results_text' => __( 'See all results', 'outsystems_blog' )
			);

			wp_localize_script(
				'swiftype', // (required) the name of the script, 'my-script' for example
				'swiftype_localize', // (required) the name of the script, 'my-script' for example
	...
	/**
		...
		*/
		public function clear_config() {
			...
			delete_option( 'swiftype_create_engine' );
			delete_option( 'swiftype_num_indexed_documents' );
			delete_post_meta_by_key( 'swiftype_imported' );
		}
